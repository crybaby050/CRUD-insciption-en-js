...
export function renderEtudiant(etudiant, { onModifier, onDesactiver }) {
    ...
    tr.innerHTML = `
        ...
        <td style="padding:14px 16px;">
            <div class="flex gap-2">
                <button class="btn-modifier w-8 h-8 rounded-lg bg-green text-white flex items-center justify-center" title="Modifier">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                    </svg>
                </button>
                <button class="btn-desactiver w-8 h-8 rounded-lg bg-red-50 text-red-500 flex items-center justify-center" title="Désactiver">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
    ...
    if (onModifier) {
        tr.querySelector(".btn-modifier").addEventListener("click", () => onModifier(etudiant.id));
    }
    
    if (onDesactiver) {
        tr.querySelector(".btn-desactiver").addEventListener("click", () => onDesactiver(etudiant.id));
    }
...
export function renderEtudiantCarte(etudiant, { onModifier, onDesactiver }) {
    ...
    if (!cardGrid) {
        console.error("cardGrid est null");
        return;
    }
    ...
    if (onModifier) {
        div.querySelector(".btn-modifier").addEventListener("click", () => onModifier(etudiant.id));
    }
    
    if (onDesactiver) {
        div.querySelector(".btn-desactiver").addEventListener("click", () => onDesactiver(etudiant.id));
    }
...
export function renderEtudiantList(etudiants, handlers) {
    ...
    etudiants.forEach(etudiant => renderEtudiant(etudiant, handlers));
}
...
export function renderEtudiantCarteList(etudiants, handlers) {
    ...
    etudiants.forEach(etudiant => renderEtudiantCarte(etudiant, handlers));
}